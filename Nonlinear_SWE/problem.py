...
class shallow_water(object):
    ...
    def initialize(self):
        ...
        self.q1 = 10.0+(rho*np.exp(-((self.X-self.Lx/2.7)**2/(2*(0.05)**2) + (self.Y-self.Ly/4)**2/(2*(0.05)**2))))
    ...
    def integrate(self):
        ...
        q1temp[:,:] = self.q1[:,:] + dt*(r1[:,:])
        q2temp[:,:] = self.q2[:,:] + dt*(r2[:,:])
        q3temp[:,:] = self.q3[:,:] + dt*(r3[:,:])
        
        r1, r2, r3 = self.right_hand_side(q1temp,q2temp,q3temp) # Note switch in sign

        q1temp[:,:] = 0.75*self.q1[:,:] + 0.25*q1temp[:,:] + 0.25*dt*r1[:,:]
        q2temp[:,:] = 0.75*self.q2[:,:] + 0.25*q2temp[:,:] + 0.25*dt*r2[:,:]
        q3temp[:,:] = 0.75*self.q3[:,:] + 0.25*q3temp[:,:] + 0.25*dt*r3[:,:]
        
        r1, r2, r3 = self.right_hand_side(q1temp,q2temp,q3temp) # Note switch in sign

        self.q1[:,:] = (1.0/3.0)*self.q1[:,:] + (2.0/3.0)*q1temp[:,:] + (2.0/3.0)*dt*r1[:,:]
        self.q2[:,:] = (1.0/3.0)*self.q2[:,:] + (2.0/3.0)*q2temp[:,:] + (2.0/3.0)*dt*r2[:,:]
        self.q3[:,:] = (1.0/3.0)*self.q3[:,:] + (2.0/3.0)*q3temp[:,:] + (2.0/3.0)*dt*r3[:,:]
