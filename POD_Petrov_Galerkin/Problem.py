...
def l_calc(u):
    '''
    Field u - shape: Nx1
    Returns d2u/dx2 - shape : Nx1
    '''
    temp_array = np.zeros(shape=(np.shape(u)[0]+2,1)) # Temp array
    ul = np.zeros(shape=(np.shape(u)[0],1)) # Array for calculating the nonlinear term
    ...
    ul[:,0] = 1.0/Rnum*(temp_array[2:,0]+temp_array[0:-2,0]-2.0*temp_array[1:-1,0])/(dx*dx)
