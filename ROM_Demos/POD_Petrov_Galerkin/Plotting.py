...
def plot_comparison(Ytilde_GP,Ytilde_DEIM,Ytilde_PGP,Ytilde):
    fig, ax = plt.subplots(ncols=3,figsize=(18,6))
    ax[0].plot(Ytilde_GP[0,:],label='POD-GP')
    ax[0].plot(Ytilde_DEIM[0,:],label='POD-DEIM')
    ax[0].plot(Ytilde_PGP[0,:],label='POD-PGP')
    ...
    ax[1].plot(Ytilde_PGP[1,:],label='POD-PGP')
    ...
    ax[2].plot(Ytilde_PGP[2,:],label='POD-PGP')
    ...
    print('POD PGP:',error_calc(Ytilde,Ytilde_PGP))
...
if __name__ == "__main__":
    ...
    POD_True = np.load('POD_True.npy')
    POD_PGP = np.load('POD_PGP.npy')
    POD_DEIM = np.load('POD_DEIM.npy')
    POD_GP = np.load('POD_GP.npy')

    plot_comparison(POD_GP,POD_DEIM,POD_PGP,POD_True)