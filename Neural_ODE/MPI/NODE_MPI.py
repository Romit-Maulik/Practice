...
def rms_prop_optimize(thetas):
    ...
    for epoch in range(num_epochs):
        ...
        if rank == 0: # Optimization is done by rank==0 only
            del_theta_g = del_theta_g/nprocs # Calculate average gradient   
            thetas = thetas - del_theta_g
            lr_counter = lr_counter + 1

            for i in range(1,nprocs):
                comm.send(thetas,dest=i,tag=i+12)
                comm.send(lr,dest=i,tag=i+13)
            if lr_counter > 100:
                lr = lr*0.9
                lr_counter = 0
        else:
            thetas = comm.recv(source=0,tag=rank+12)
            lr = comm.recv(source=0,tag=rank+13)
